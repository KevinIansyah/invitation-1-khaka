...
import type { Invitation, Wish } from '@/types';
...
export function DataTableWish({ data, pageIndex, setPageIndex, totalPages, totalItems, perPage, initialFilters = {} }: DataTableProps) {
            ...
            cell: ({ row }) => {
                return (
                    <div className="flex items-center gap-1">
                        <AlertDeleteDialog
                            description={`Tindakan ini tidak dapat dibatalkan. Data ucapan "${row.original.name}" akan dihapus secara permanen dari sistem.`}
                            deleteUrl={`/dashboard/wish/${row.original.id}`}
                            tooltipText="Hapus Ucapan"
                        />
                    </div>
                );
            },
        ...
        <div className="w-full">
            <div className="flex flex-col items-center justify-between gap-2 pb-4 md:flex-row">
                <InputGroup className="max-w-sm">
                    <InputGroupInput placeholder="Filter nama atau ucapan..." value={searchValue} onChange={handleSearchChange} />
                    <InputGroupAddon>
                        <Search />
                    </InputGroupAddon>
                </InputGroup>
